...
def check_symbol(symbol):
    if not isinstance(symbol, str):
        raise BinanceApiException(
            BinanceApiException.INPUT_ERROR, '[Input] symbol must be string')
    if re.match(reg_ex, symbol):
        raise BinanceApiException(
            BinanceApiException.INPUT_ERROR, '[Input] ' + symbol + '  is invalid symbol')


def check_symbol_list(symbols):
    if not isinstance(symbols, list):
        raise BinanceApiException(
            BinanceApiException.INPUT_ERROR, '[Input] symbols in subscription is not a list')
...
def check_currency(currency):
    if not isinstance(currency, str):
        raise BinanceApiException(
            BinanceApiException.INPUT_ERROR, '[Input] currency must be string')
    if re.match(reg_ex, currency) is not None:
        raise BinanceApiException(
            BinanceApiException.INPUT_ERROR, '[Input] ' + currency + '  is invalid currency')
...
def check_should_not_none(value, name):
    if value is None:
        raise BinanceApiException(
            BinanceApiException.INPUT_ERROR, '[Input] ' + name + ' should not be null')


def check_should_none(value, name):
    if value is not None:
        raise BinanceApiException(
            BinanceApiException.INPUT_ERROR, '[Input] ' + name + ' should be null')
...
def format_date(value, name):
    ...
    if not isinstance(value, str):
        raise BinanceApiException(
            BinanceApiException.INPUT_ERROR, '[Input] ' + name + ' must be string')
    try:
        new_time = time.strptime(value, '%Y-%m-%d')
        return time.strftime('%Y-%m-%d', new_time)
    except:
        raise BinanceApiException(
            BinanceApiException.INPUT_ERROR, '[Input] ' + name + ' is not invalid date format')
